...
- **`supplier_searches`** — Supplier Agent output per run. Unlike `review_insights`,
  ...
  recording it triggers no other action). Also `landed_cost_mxn_manual` (numeric,
  nullable), `landed_cost_source` (text, nullable — only `'manual'` or `null`),
  `landed_cost_notes` (text, nullable), `landed_cost_captured_at` (timestamptz,
  nullable) — the Procurement Agent's manual landed-cost capture, 1:1 with the row
  (added in Fase 4). Restricted to `service_role`, same as the other agent tables.
- **`procurement_documents`** — RFQ/PO documents generated by the Procurement Agent.
  Unlike the landed-cost columns above, RFQ and PO can each be regenerated multiple
  times over the same `supplier_searches` row, so they live in their own table instead
  of a merged field. `id`, `created_at`, `supplier_search_id` (FK →
  `supplier_searches.id`, `on delete cascade`, required), `document_type` (`'rfq' |
  'po'`), `content` (jsonb — `{ asunto, cuerpo }`, plus `campos_solicitados` for RFQ).
  Restricted to `service_role`.
- **`buy_simulations`** — Buy Simulator output per run. `id`, `created_at`,
  `product_candidate_id` (FK → `product_candidates.id`, `on delete cascade`),
  `supplier_search_id` (FK → `supplier_searches.id`, `on delete cascade`),
  `capital_disponible_mxn`, `unidades_por_mes_asumidas` (the sell-through assumption —
...
Three endpoints, all logged to `agent_runs` with `agent_name: 'procurement_agent'`, all
requiring a `supplier_searches` row with `selected_supplier_id` already set (the
continuation of the selection made in Fase 3, not an independent flow) — never sends
anything to the supplier automatically:

`POST /api/agents/procurement/rfq` — `{ supplier_search_id: string }`.
1. Fetches the `supplier_searches` row. 404 if it doesn't exist; 422 (no `agent_runs` row
   logged, same pattern as the Supplier Agent's veredicto check) if
   `selected_supplier_id` is null.
2. Finds the selected option inside `options` (by `supplierId`), and fetches the
   associated candidate's `raw_data.catalog` as `productSpecs`.
3. Calls `generateRfqDraft(productSpecs, selectedOption)` (`src/lib/claude-analysis.ts`,
   same `messages.parse` + `zodOutputFormat` pattern as the other four analysis
   functions). The RFQ is drafted **in English** — Alibaba suppliers operate
   predominantly in English — translating the (often Spanish) `raw_data.catalog` specs
   rather than copying them, and explicitly requests confirmed unit price, freight cost,
   lead time, and payment terms, while stating the Alibaba price is only a starting
   point.
4. Inserts the result into `procurement_documents` (`document_type: 'rfq'`).

`PATCH /api/agents/supplier/[id]/landed-cost` — `{ landed_cost_mxn: number, notes?:
string }`. Records the real landed cost the user obtained from the supplier's quote —
same pattern as `cost_manual` on the Product Analyst Agent: a number the human
enters/calculates, never inferred by the system. 400 if `landed_cost_mxn` isn't a
positive number. Updates `landed_cost_mxn_manual`, `landed_cost_source: 'manual'`,
`landed_cost_notes`, `landed_cost_captured_at` on the `supplier_searches` row. This is
the value the Buy Simulator requires before it will run.

`POST /api/agents/procurement/po` — `{ supplier_search_id: string, quantity: number,
unit_price_usd: number, notes?: string }`. Same existence/selection checks as the RFQ
endpoint. `quantity` and `unit_price_usd` are the figures the user confirmed manually
after receiving the supplier's real quote — **never** read from the original Alibaba
`priceBreaks`, since a formal quote's price/quantity usually differ from the initial
listing. Calls `generatePoTemplate()` (same file/pattern) and inserts the result into
`procurement_documents` (`document_type: 'po'`) — the generated PO is explicitly a
reference template, never a legally binding document.

### Buy Simulator

`POST /api/agents/buy-simulator` — `{ product_candidate_id: string, supplier_search_id:
string, capital_disponible_mxn: number, unidades_por_mes_asumidas: integer }`.
**No Claude call** — deterministic arithmetic only (`buildBuyScenarios()` in
`src/lib/buy-simulator.ts`), since there's no qualitative judgment involved. Still logged
to `agent_runs` with `agent_name: 'buy_simulator'` — traceability doesn't depend on
whether a run involved an LLM.

1. Fetches the `supplier_searches` row (400 if it doesn't belong to the given
   `product_candidate_id`). If `landed_cost_mxn_manual` is null, returns 422 explicitly
   telling the user to capture it first via the Procurement Agent — **never** simulates
   silently with the partial (freight/duties-less) landed cost from Fase 3.
2. Reads the sale price from
   `raw_data.analyst.competitivePricing.buyBoxNewPrice.Amount` on the candidate — **not**
   from `raw_data.analyst.verdict.variables.precio.valor`, which is human-readable prose
   (e.g. `"$456.90 MXN (Buy Box New, envío $0)"`), not a reliably parseable number. 422 if
   that price is missing.
3. Takes the selected option's `priceBreaks` (from `supplier_searches.options`, confirmed
   present there directly — no need to dig into `raw_results`) as the candidate quantity
   tiers.
4. `unidades_por_mes_asumidas` (sell-through) is **always a manual user input** — never
   estimated by the system, since `sales_estimate`/`revenue_estimate` remain `sin_dato`
   without a Keepa-equivalent source (Fase 1.5). Inventing a sell-through here would
   violate the "never fabricate data" principle.
5. `buildBuyScenarios()` returns up to 4 scenarios (one per price-break tier whose
   `inversionTotalMxn` fits within `capital_disponible_mxn`), each with
   `cantidadComprada`, `inversionTotalMxn`, `revenueEsperadoMxn`, `profitEsperadoMxn`,
   `mesesParaVenderTodo`, and `mesesParaRecuperarCapital`. An empty array (not even the
   cheapest/MOQ tier fit the declared capital) is a valid result, not an error — the
   route responds 200 with an explicit `nota` instead of a 500.
